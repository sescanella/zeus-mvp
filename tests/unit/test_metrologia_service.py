...
The 3-cycle limit (CycleCounterService and the BLOQUEADO escalation) was
removed, so a RECHAZADO always writes the same plain marker and there's no
cycle counter to inject.
"""
import pytest
...
@pytest.mark.asyncio
async def test_completar_rechazado_writes_plain_marker(metrologia_service, mock_sheets_repo, ready_spool):
    """RECHAZADO writes the canonical plain marker. No cycle suffix, no BLOQUEADO escalation."""
    mock_sheets_repo.get_spool_by_tag = Mock(return_value=ready_spool)
    ...
    assert "BLOQUEADO" not in estado["value"]


@pytest.mark.asyncio
async def test_completar_aprobado_writes_aprobado_marker(metrologia_service, mock_sheets_repo, ready_spool):
    """APROBADO writes 'METROLOGIA APROBADO ✓' (no cycle reset path anymore)."""
    mock_sheets_repo.get_spool_by_tag = Mock(return_value=ready_spool)
    ...
    updates = mock_sheets_repo.batch_update_by_column_name.call_args.kwargs["updates"]
    estado = next(u for u in updates if u["column_name"] == "Estado_Detalle")
    assert "APROBADO" in estado["value"]
    assert "Ciclo" not in estado["value"]
...
async def test_completar_continues_on_metadata_failure(metrologia_service, mock_sheets_repo, mock_metadata_repo, ready_spool):
    """Operation should succeed even if metadata logging fails (best-effort)."""
    mock_sheets_repo.get_spool_by_tag = Mock(return_value=ready_spool)
