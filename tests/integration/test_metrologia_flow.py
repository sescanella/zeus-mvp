...
# ============================================================================
# Legacy BLOQUEADO tolerance — sheets pre-existing the 3-cycle removal
# ============================================================================


@pytest.mark.asyncio
async def test_legacy_bloqueado_spool_can_be_repaired(mock_sheets_repo, mock_metadata_repo):
    """A sheet row left behind with `estado_detalle="BLOQUEADO - Contactar supervisor"`
    (legacy from the removed 3-cycle limit) must be accepted by reparación as if
    it were RECHAZADO — no SpoolBloqueadoError, no 403."""
    from backend.services.reparacion_service import ReparacionService
    from backend.models.spool import Spool
    from datetime import date as _date

    legacy_bloqueado_spool = Spool(
        tag_spool="INTEGRATION-001",
    ...
    # Should not raise: legacy BLOQUEADO is normalized to RECHAZADO and the
    # spool is taken for repair.
