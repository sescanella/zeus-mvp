...
interface NotasModalProps {
  isOpen: boolean;
  spool: SpoolCardData;
  workerId: number | null; // optional author hint: if the spool is occupied, that worker signs the note; otherwise the note is saved anonymously.
  onClose: () => void;
 ...
 * NotasModal — v5.1 F-1.
 *
 * Read + append workflow for the per-spool `Notas` column. History is
 * shown read-only (pre-wrap preserves newlines and the `YYYYMMDD:`
 * prefixes written by the backend).
 *
 * Author attribution is optional: if `workerId` is provided (spool
 * occupied), that worker is sent as the author and recorded in the audit
 * trail. If `workerId` is null, the note is still saved — the backend
 * records the author as ANONIMO.
 */
