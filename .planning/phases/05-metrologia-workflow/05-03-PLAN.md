...
<task type="auto">
  ...
  <action>
    Update operation selection page to handle METROLOGIA differently:

    1. Add METROLOGIA as third operation option (alongside ARM and SOLD)
    2. In handleSelectOperation function:
       - If operation === 'METROLOGIA', navigate directly to '/seleccionar-spool'
       - Pass query param or context flag to indicate metrologia flow
       - ARM/SOLD continue to '/tipo-interaccion' as before

    3. Visual: Add METROLOGIA button with appropriate icon/color
       - Use same h-20 height for mobile-first design
       - Consider purple or blue color to differentiate from ARM/SOLD

    This skips the INICIAR/COMPLETAR choice entirely for instant inspection.
  </action>
  <verify>npm run build && echo "TypeScript compilation successful"</verify>
  <done>METROLOGIA operation bypasses tipo-interaccion, goes directly to spool selection</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    3. Update seleccionar-spool page:
       - When operacion === 'METROLOGIA', navigate to '/resultado-metrologia' after spool selection
       - Instead of going to /confirmar
