...
<task type="auto">
  ...
  <verify>
    1. First run dry-run: python backend/scripts/add_estado_detalle_column.py --dry-run
    2. Then execute actual column addition: python backend/scripts/add_estado_detalle_column.py
    3. Verify in Google Sheets that column 67 is labeled "Estado_Detalle"
  </verify>
  <done>Column 'Estado_Detalle' successfully added at position 67 in production sheet and visible in Google Sheets UI</done>
</task>
