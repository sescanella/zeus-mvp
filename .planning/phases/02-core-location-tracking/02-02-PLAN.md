...
<task type="auto">
  ...
  <action>
    ...
    1. TOMAR operation:
       ...
       - Log TOMAR event to Metadata (evento_tipo: "TOMAR_{operacion}")
       - Return success with "Spool {tag} tomado por {worker}"

    2. PAUSAR operation:
       - Verify worker owns the lock (compare Redis lock owner)
       - Update spool state to "ARM parcial" or "SOLD parcial" in sheet
    ...
    3. COMPLETAR operation:
       ...
       - Return success with "Operación completada en {tag}"
  ...
  <done>OccupationService orchestrates locks, sheets updates, and metadata logging</done>
</task>
...
- Metadata maintains complete audit trail
