...
must_haves:
  truths:
    - "Concurrent updates to same spool resolve without data corruption"
    - "Workers see clear error messages when conflicts exceed retry limit"
    - "System maintains data consistency under heavy concurrent load"
    - "No worker loses work due to race conditions"
  artifacts:
...
<task type="auto">
  <name>Task 2: Extend sheets repository with version column using dynamic header mapping</name>
  <files>backend/repositories/sheets_repository.py</files>
  <action>
    Add version token support to SheetsRepository:

    1. Extend column mapping to include "version" column:
       - Use dynamic header mapping by name: headers["version"]
       - NEVER use hardcoded column index
       - Follow existing pattern from _get_column_index() method

    2. Add version-aware update method:
       - update_spool_with_version(tag_spool, updates: dict, expected_version: str)
       - Read current version from sheet using headers["version"]
    ...
    3. Add version reading:
       ...
       - Use headers["version"] to find column index dynamically
...
2. Sheet updates validate version using dynamic header mapping
...
- No hardcoded column indices in any code
</success_criteria>
