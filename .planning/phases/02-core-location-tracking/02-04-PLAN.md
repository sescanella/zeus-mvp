...
<task type="auto">
  ...
  <verify>pytest tests/integration/test_race_conditions.py::test_concurrent_tomar_prevents_double_booking -v (should FAIL initially)</verify>
  <done>Integration test fails, proving we need race condition prevention</done>
</task>
...
<verification>
1. Race condition test proves only 1 of 10 concurrent requests succeeds
2. Redis lock tests validate atomic SET NX EX behavior
3. Occupation service tests verify business rules
4. Conflict service tests confirm retry logic works
