...
must_haves:
  truths:
    - "REPARACIÓN appears as 4th operation button on frontend"
    - "Spool selection shows RECHAZADO spools with cycle info"
    - "BLOQUEADO spools visible but disabled in UI"
    - "API endpoint filters and returns reparación spools correctly"
  artifacts:
    ...
    - path: "zeues-frontend/app/operacion/page.tsx"
      provides: "REPARACIÓN button in operation selection"
      min_lines: 20
  key_links:
    ...
    - from: "backend/routers/spools.py"
      to: "CycleCounterService"
      via: "service injection and usage"
      pattern: "cycle_counter\\.extract_cycle_count"
---

<objective>
Create REST endpoints for reparación workflow and integrate frontend with 4th operation module, showing RECHAZADO spools with cycle info and BLOQUEADO state.

Purpose: Enable workers to select and repair rejected spools through the UI with proper visual feedback
Output: Working REST API and frontend integration for reparación workflow

Note: Per Phase Context, no role display needed - all active workers can access REPARACIÓN module
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    1. In routers/spools.py, add GET /api/spools/reparacion:
       ...
       - For each spool:
         * Parse cycle: current_cycle = cycle_counter.extract_cycle_count(spool.estado_detalle)
         * Check if blocked: is_blocked = cycle_counter.should_block(current_cycle)
    ...
    2. In routers/actions.py, add reparación actions:
       - Inject both ReparacionService and CycleCounterService:
         ```python
         from backend.services import ReparacionService
         from backend.services.cycle_counter_service import CycleCounterService
         ```
       ...
       - Handle SpoolBloqueadoError → 403 response

    3. In main.py, add exception handler:
       - Map SpoolBloqueadoError to 403 Forbidden status

    4. Add get_cycle_counter_service to dependency injection:
       ```python
    ...
    Explicit service injection and usage shown above
  </action>
  <verify>curl -X GET http://localhost:8000/api/spools/reparacion | jq</verify>
  <done>REST endpoints return RECHAZADO spools with bloqueado flags using CycleCounterService</done>
</task>

<task type="auto">
  <name>Task 2: Add REPARACIÓN to frontend operation selection</name>
  <files>zeues-frontend/app/operacion/page.tsx</files>
  <action>
    Add REPARACIÓN as 4th operation option:
    ...
    1. Add to operations array:
       ...
       const operations = [
         ...
         { id: 'REPARACION', label: 'Reparación', color: 'bg-yellow-600' }
       ];
       ```
    ...
    3. Style the button:
       ...
       - Icon: 🔧 or similar repair symbol

    Note: No role display needed per CONTEXT.md decision - all workers access REPARACIÓN
  ...
  <done>REPARACIÓN button appears as 4th operation with yellow styling</done>
</task>

<task type="auto">
  <name>Task 3: Update spool selection for reparación</name>
  <files>zeues-frontend/app/seleccionar-spool/page.tsx, zeues-frontend/components/SpoolCard.tsx</files>
  <action>
    Modify spool selection to handle reparación workflow:

    1. In seleccionar-spool/page.tsx:
       - Detect when operacion === 'REPARACION'
       - Call getSpoolsReparacion() instead of generic fetch
       - Show title: "Seleccionar Spool para Reparación"
    ...
    2. Create/Update SpoolCard component:
       ...
       - If bloqueado:
         ...
         * Show lock icon: 🔒 BLOQUEADO - Contactar supervisor
       - If not bloqueado:
         * Show cycle warning: ⚠️ Ciclo X/3
         * Yellow warning color
       - Disable onClick when bloqueado

    3. Handle empty state:
       - Message: "No hay spools rechazados pendientes de reparación"

    Note: No responsible role display - per CONTEXT.md all workers can repair
...
<task type="auto">
  <name>Task 4: Add API functions for reparación</name>
  <files>zeues-frontend/lib/api.ts</files>
  <action>
    Add reparación API functions following existing patterns:
...
<verification>
# Backend: Test reparación endpoints
...
npm run dev
# Navigate to /operacion - should see REPARACIÓN button
</verification>

<success_criteria>
- [ ] GET /api/spools/reparacion uses injected CycleCounterService
- [ ] POST endpoints handle reparación actions (tomar/pausar/completar/cancelar)
- [ ] REPARACIÓN appears as 4th yellow button on operation selection
- [ ] Spool cards show cycle info and disable BLOQUEADO spools
- [ ] TypeScript compilation passes with no 'any' types
- [ ] Frontend build succeeds
- [ ] No role display in UI (all workers can repair per CONTEXT.md)
</success_criteria>
