...
<task type="auto">
  ...
  <action>
    ...
    1. Inject dependencies: sheets_repo, metadata_repo, redis_repo (for events)

    2. Implement core methods:
       - tomar_reparacion(tag_spool, worker_id, worker_nombre):
         * Check not BLOQUEADO (defer validation to next plan)
    ...
    Cycle parsing will be added in next plan with CycleCounterService
  </action>
  <verify>python -c "from backend.services import ReparacionService; print('Service imported successfully')"</verify>
  <done>ReparacionService created with TOMAR/PAUSAR/COMPLETAR/CANCELAR methods</done>
</task>
