...
<objective>
Create version detection service to identify v3.0 vs v4.0 spools based on union count.

Purpose: Enable dual workflow routing by detecting spool version from Total_Uniones column.
Output: Service that queries column 68 with retry logic and diagnostic endpoint for transparency.
</objective>
...
<task type="auto">
  ...
  <verify>python -c "from backend.models.version import VersionInfo, VersionResponse; print('Models imported')"</verify>
  <done>Version models define API response structure</done>
</task>

<task type="auto">
  <name>Task 3: Create diagnostic endpoint</name>
