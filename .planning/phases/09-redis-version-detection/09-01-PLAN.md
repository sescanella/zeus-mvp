...
must_haves:
  ...
  artifacts:
    - path: "backend/services/redis_lock_service.py"
      provides: "Persistent lock acquisition with PERSIST command"
      min_lines: 400
...
<objective>
Implement persistent Redis locks without TTL to support long-running work sessions (5-8 hours).

Purpose: Enable workers to hold locks indefinitely during extended work periods without timeout issues.
Output: Modified Redis lock service with two-step acquisition (SET with safety TTL, then PERSIST).
</objective>
...
<task type="auto">
  <name>Task 3: Add unit tests for persistent locks</name>
  ...
  <done>Unit tests pass for persistent lock acquisition, persist failure, and backward compatibility</done>
</task>
...
- [ ] Unit tests pass for all persistent lock scenarios
</success_criteria>
