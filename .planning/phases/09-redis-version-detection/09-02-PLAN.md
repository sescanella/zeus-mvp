...
must_haves:
  truths:
    ...
    - "Cleanup operations are silent (no Metadata events)"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    6. If age > 24 hours:
       ...
       - If ocupado_por is None or "DISPONIBLE":
         ...
         - Log cleanup action to application logger ONLY (not Metadata)
    ...
    Important: Log to Python logger ONLY, do NOT create Metadata events.
    Reference: 09-RESEARCH.md Pattern 2 for implementation details.
    Reference: User decision "Silent cleanup (no Metadata events for cleanup operations)"
  ...
  <done>lazy_cleanup_one_abandoned_lock() scans and cleans one abandoned lock, logs to application logger only, no Metadata events created</done>
</task>
...
<task type="auto">
  ...
  <verify>pytest tests/unit/test_lazy_cleanup.py::test_iniciar_calls_cleanup -v</verify>
  <done>INICIAR/tomar calls lazy cleanup before lock acquisition, functional test verifies cleanup is invoked</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    6. Test cleanup logs to application logger, not Metadata
    7. Test iniciar_calls_cleanup() - verify INICIAR endpoint invokes cleanup
    ...
    - metadata_service to ensure NO events created

    Verify cleanup deletes correct locks and logs appropriately.
    Verify NO Metadata events are created during cleanup.
  </action>
  <verify>pytest tests/unit/test_lazy_cleanup.py -v</verify>
  <done>Unit tests pass for all lazy cleanup scenarios, verify no Metadata events</done>
</task>
...
- Cleanup logs to application logger, NOT Metadata
</verification>
...
- [ ] Cleanup logs to Python logger only (no Metadata events)
- [ ] INICIAR/tomar endpoint calls cleanup inline (not async)
- [ ] Unit tests verify one-lock-per-operation behavior
- [ ] Unit tests verify NO Metadata events created during cleanup
</success_criteria>
