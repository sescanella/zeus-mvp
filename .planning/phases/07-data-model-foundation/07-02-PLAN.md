...
must_haves:
  truths:
    - "System can log granular union-level events in Metadata"
    - "Uniones sheet structure is validated for v4.0 compatibility"
    - "System can track both spool-level and union-level changes"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    **IMPORTANT CONTEXT:** Research (lines 14-18) indicates that the Uniones sheet already exists with 18 columns including audit fields at positions 14-18. This script validates the existing structure, not adds new columns.
    ...
    - Read headers from row 1 using worksheet.row_values(1)
    - Compare against expected schema (normalize names for comparison)
    - Report any missing or extra columns with clear messages
    - Exit with error code 0 if validation passes, 1 if fails
    - Support --fix flag to attempt adding missing columns (headers only)

    Important: This script validates the EXISTING structure. The audit columns (14-18) already exist per research findings. This is a validation script, not a migration script.
  </action>
  <verify>python backend/scripts/validate_uniones_sheet.py && echo "Validation passed" || echo "Validation failed"</verify>
  <done>Script reports either "Uniones sheet valid: 18 columns found" or lists specific structural issues</done>
</task>
...
<task type="auto">
  ...
  <verify>python backend/scripts/extend_metadata_schema.py --dry-run && echo "Script ready" || echo "Script failed"</verify>
  <done>Script reports "1 column would be added to Metadata sheet" or "N_UNION column already exists"</done>
</task>
...
<success_criteria>
- [ ] Uniones validation script correctly validates existing 18-column structure
