...
<task type="auto">
  ...
  <action>
    ...
    7. **CRITICAL:** Call ColumnMapCache.invalidate("Operaciones") after adding columns to ensure the cache rebuilds with new column mappings

    Follow the pattern from backend/scripts/add_estado_detalle_column.py which added column 67.

    Important cache invalidation step:
    ```python
    from backend.core.column_map_cache import ColumnMapCache
    # After successful column addition:
    ColumnMapCache.invalidate("Operaciones")
    print("Cache invalidated for Operaciones sheet")
    ```
  </action>
  <verify>python backend/scripts/extend_operaciones_schema.py --dry-run && python -c "from backend.scripts.extend_operaciones_schema import main; import inspect; print('Cache invalidation call found' if 'ColumnMapCache.invalidate' in inspect.getsource(main) else 'WARNING: Missing cache invalidation')"</verify>
  <done>Script executes without errors, reports "5 columns would be added" in dry-run mode, and includes cache invalidation</done>
</task>
...
2. Verify script would add columns if they don't exist: Check output for "5 columns would be added" or "Columns already exist"
3. Verify ColumnMapCache has invalidation methods: `grep "def invalidate" backend/core/column_map_cache.py`
4. Verify cache invalidation is called in script: `grep "ColumnMapCache.invalidate" backend/scripts/extend_operaciones_schema.py`
</verification>
...
- [ ] Script calls ColumnMapCache.invalidate() after changes
</success_criteria>
