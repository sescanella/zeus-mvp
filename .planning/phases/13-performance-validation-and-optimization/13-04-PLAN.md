...
<task type="auto">
  ...
  <action>
    ...
    1. Global metrics collectors:
       - Import RateLimitMonitor from backend/utils/rate_limiter.py (created in 13-03)
       - Import numpy for percentile calculation (installed in 13-01)
    ...
    3. Real-time metric calculation:
       ...
       - Calculate running statistics using numpy.percentile()

    4. Comprehensive reporting:
       - Latency: avg, p50, p95, p99, max (using numpy)
       - API efficiency: calls per operation
       - Rate compliance: current RPM, quota usage (via RateLimitMonitor)
       - Memory: baseline, peak, increase
       - Errors: categorized by type

    Wire together the RateLimitMonitor from 13-03 and numpy from 13-01 for comprehensive metrics.
  </action>
  <verify>
