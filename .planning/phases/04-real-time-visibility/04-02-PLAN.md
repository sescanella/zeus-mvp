...
<task type="auto">
  ...
  <action>
    ...
    1. Update OccupationService constructor to accept redis_event_service
    2. In tomar_spool() after successful lock + Sheets write:
       - Publish TOMAR event with tag_spool, worker_nombre, estado_detalle
    3. In pausar_spool() after releasing lock:
       - Publish PAUSAR event with tag_spool
    4. In completar_operacion() after state update:
       - Publish COMPLETAR event with tag_spool, operation, worker_nombre
...
<task type="auto">
  ...
  <action>
    ...
    1. Update StateService constructor to accept redis_event_service
    2. In completar() after state machine transition:
       - Call EstadoDetalleBuilder to get new estado_detalle
       - Publish STATE_CHANGE event with updated estado_detalle
    3. Update get_state_service() to inject RedisEventService
