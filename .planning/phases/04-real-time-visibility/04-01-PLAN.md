...
files_modified:
  ...
  - backend/tests/unit/test_redis_event_service.py
...
<task type="auto">
  ...
  <verify>pip freeze | grep sse-starlette==3.2.0</verify>
  <done>sse-starlette added to requirements and installed</done>
</task>

<task type="auto">
  <name>Task 2: Create Redis event publisher service with tests</name>
  <files>backend/services/redis_event_service.py, backend/tests/unit/test_redis_event_service.py</files>
  <action>
    1. Create RedisEventService class that publishes state change events to Redis pub/sub.
    ...
    2. Create unit tests in backend/tests/unit/test_redis_event_service.py:
    - Test publish_spool_update publishes correct JSON structure
    - Test all event types (TOMAR, PAUSAR, COMPLETAR, STATE_CHANGE)
    - Mock Redis client to verify publish calls
  ...
  <done>RedisEventService publishes events to Redis pub/sub channel with unit tests passing</done>
</task>
