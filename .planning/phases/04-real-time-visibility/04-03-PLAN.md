...
must_haves:
  truths:
    - "Worker sees updates appear without manual refresh"
    - "Connection status indicator shows red when disconnected, green when connected"
    - "Spool list updates automatically when other workers take or release spools"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    3. Handle events with onMessage callback that processes:
       ...
       - COMPLETAR: Remove spool from available list (operation completed)
  ...
  <verify>grep -E "onMessage.*TOMAR.*PAUSAR.*COMPLETAR" zeues-frontend/app/seleccionar-spool/page.tsx</verify>
  <done>Spool selection updates in real-time with SSE events and handles all event types</done>
</task>
...
5. All event types (TOMAR, PAUSAR, COMPLETAR, STATE_CHANGE) handled in onMessage
</verification>
