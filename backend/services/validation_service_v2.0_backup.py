...
v2.1 Direct Read:
- Estados ARM/SOLD se leen directamente desde columnas de Operaciones
- NO reconstruye desde Metadata (más simple y confiable)
- Validaciones basadas en presencia de datos:
  * ARM PENDIENTE: armador = None
  * ARM EN_PROGRESO: armador != None AND fecha_armado = None
  * ARM COMPLETADO: fecha_armado != None
  * SOLD PENDIENTE: soldador = None
  * SOLD EN_PROGRESO: soldador != None AND fecha_soldadura = None
...
class ValidationService:
    ...
    v2.1: Lee estados directamente desde columnas de Operaciones (NO usa Metadata).
    """

    def __init__(self, role_service=None):
        """
        Inicializa el servicio de validación (v2.1 Direct Read).

        Args:
            role_service: Servicio para validar roles operativos (opcional para tests)
        """
